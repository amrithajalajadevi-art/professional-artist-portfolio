...
export async function sendEmail(
  ...
  const rawData = {
    ...
    website_url_bot_check: formData.get("website_url_bot_check"),
  };

  // 1. HONEYPOT SECURITY LAYER: Check if hidden bot field was filled out
  const honeypot = rawData.website_url_bot_check;
  if (honeypot && typeof honeypot === "string" && honeypot.trim() !== "") {
    console.warn("Spam bot submission caught by honeypot field.");
    // Return fake success response to fool the spam bot without dispatching email
    return {
      success: true,
  ...
  // 2. ZOD VALIDATION & SANITIZATION LAYER
    ...
    const data = await resend.emails.send({
      ...
      html: `
        <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px; color: #333333; line-height: 1.6;">
          ...
          <div style="white-space: pre-wrap; background: #F7F4F0; padding: 24px; border: 1px solid #E8E2DA; border-radius: 4px; font-size: 15px; color: #444444;">${safeMessage}</div>
