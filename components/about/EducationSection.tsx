...
export function EducationSection({ education, affiliations }: EducationSectionProps) {
  const hasEducation = Boolean(education && education.length > 0);
  const hasAffiliations = Boolean(affiliations && affiliations.length > 0);

  // If neither document/item exists, do not render the component or title at all
  if (!hasEducation && !hasAffiliations) {
    return null;
  }

  // Dynamic heading based on which documents were found
  const sectionTitle =
    hasEducation && hasAffiliations
  ...
  return (
    <section className="p-8 sm:p-12 xl:p-16 bg-[#F7F4F0] space-y-10">
      ...
      >
        {/* Education Timeline — only displayed if education documents exist */}
        {hasEducation && (
            ...
            <FadeInStagger staggerDelay={0.15}>
              <div className="space-y-4 font-sans text-xs sm:text-sm">
                {education.map((item, idx) => (
                  <div
                    key={idx}
                    className="space-y-1 pb-3 border-b border-[#E8E2DA] last:border-b-0"
                  >
                    <span className="text-[#8A7976] font-sans text-xs block">
                      {item.year}
                    </span>
        ...
        {/* Professional Affiliations — only displayed if affiliations documents exist */}
        {hasAffiliations && (
          <div className="space-y-6">
            <FadeIn direction="up">
              <h3 className="font-serif text-xl font-normal uppercase text-[#4A2E35]">
              ...
              <div className="space-y-4 font-sans text-xs sm:text-sm">
                {affiliations.map((item, idx) => (
                  ...
                  >
                    ...
                    <p className="text-[#8A7976]">{item.organization}</p>
                    {item.details && (
