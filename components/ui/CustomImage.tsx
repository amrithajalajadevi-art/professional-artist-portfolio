...
export function CustomImage({
  ...
  return (
    <figure className={`relative overflow-hidden bg-zinc-100/80 ${aspectClass} ${containerClassName}`}>
      {/* Loading Skeleton Placeholder */}
      {isLoading && (
        <div 
          className="absolute inset-0 z-10 bg-gradient-to-r from-zinc-100 via-zinc-200/50 to-zinc-100 bg-[length:200%_100%] animate-pulse"
          aria-hidden="true"
        />
      )}
      ...
      {hasError ? (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-100 text-zinc-400 p-4 text-center">
          ...
          <span className="text-xs font-sans font-medium">{alt || "Artwork Image"}</span>
        </div>
      ) : (
        <Image
          src={src}
